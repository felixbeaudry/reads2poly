...
{ if ((($sequence_names[$x] =~ /NM2/) or ($sequence_names[$x] =~ /NM3/) or ($sequence_names[$x] =~ /NM5/) or ($sequence_names[$x] =~ /NM6/)) )
{$data[$r]=$totdata[$x];
$r+=1;
}
}
	...
	$r=1;
	for ($x=0; $x<scalar(@sequence_names); ++$x){
		if ((($sequence_names[$x] =~ /TM/) || ($sequence_names[$x] =~ /TX/) || ($sequence_names[$x] =~ /OK/))  ){
			$data[$r]=$totdata[$x];
			$r+=1;
}	}	}
...
{ $r=1;
for ($x=0; $x<scalar(@sequence_names); ++$x)
{ if ((($sequence_names[$x] =~ /NM2/) or ($sequence_names[$x] =~ /NM3/) or ($sequence_names[$x] =~ /NM5/) or ($sequence_names[$x] =~ /NM6/)) and ($sequence_names[$x] =~ /_Y/))
{$data[$r]=$totdata[$x];
$r+=1;
}
}
...
{ $r=1;
for ($x=0; $x<scalar(@sequence_names); ++$x)
{if ((($sequence_names[$x] =~ /NM1/) or ($sequence_names[$x] =~ /NM4/)) and ($sequence_names[$x] =~ /_X/))
{$data[$r]=$totdata[$x];
$r+=1;
}
}
...
{ $r=1;
for ($x=1; $x<scalar(@sequence_names); ++$x)
{if ((($sequence_names[$x] =~ /NM2/) or ($sequence_names[$x] =~ /NM3/) or ($sequence_names[$x] =~ /NM5/) or ($sequence_names[$x] =~ /NM6/)) and ($sequence_names[$x] =~ /_X/))
{$data[$r]=$totdata[$x];
$r+=1;
}
}
...
$r=1;
for ($x=1; $x<scalar(@sequence_names); ++$x)
{if (($sequence_names[$x] =~ /9/) or ($sequence_names[$x] =~/10/))
{$data[$r]=$totdata[$x];
$r+=1;
}
}
...
print "$a1 totalS: $totsnps samsize: $numseqs theta: $thettot pi: $totpi tajima's D: $TajD_tot";

$loc=substr($file,0,20);

print OUT2 $loc, "\t", $numseqs-1, "\t", $no_syn_codons,"\t", $thetaS, "\t", $no_polyS, "\t", $no_polyS_freq, "\t", $no_divS, "\t", $pi_syn_site, "\t", $pi_JC_syn, "\t", $Dxy_syn, "\t", $Dxy_JC_syn, "\t", $no_syn_fourfold_codons, "\t", $D_fourfold, "\t", $D_JC_fourfold, "\t", $TajD_syn, "\t",  $FuLiD_syn,  "\t";

print OUT2			                "\t", $no_rep_codons,"\t",$thetaR, "\t", $no_polyR, "\t", $no_polyR_freq, "\t", $no_divR, "\t", $pi_rep_site, "\t", $pi_JC_rep, "\t", $Dxy_rep, "\t", $Dxy_JC_rep, "\t",																		   $TajD_rep, "\t",  $FuLiD_rep;
...
else {print OUT2 "0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t 0 \t";}
