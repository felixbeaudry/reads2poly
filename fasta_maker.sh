...
#run with: bash /ohta/felix.beaudry/scripts/reads2poly/fasta_maker.sh inds.list loci.list allFasta cutoff T/F
...
perl /ohta/felix.beaudry/scripts/reads2poly/vcf2fasta_uni.pl  -v ${ind}.uni.vcf -o ${ind} -l ${ind}.logfile -a T -m ${cutoff} 2>${ind}_vcferrors.txt
fi
while read loc
do
echo -e "${ind}\t${loc}"
